...
| `PUT`/`POST` | `/v1/dental/ledi/batches/upload-zip/chunk` | **preferido na UI:** corpo `application/octet-stream` (fatia 1–2 MB) + query `uploadId`, `index`, `total`, `fileName`, `expectedTipo`, `totalBytes`. Junta em disco; última fatia ingere o ZIP (job async se ≥1500 XMLs). |
