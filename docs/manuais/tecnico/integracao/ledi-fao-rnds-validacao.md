...
| `POST` | `/v1/dental/ledi/batches/upload-zip` | multipart `file` (.zip) + `expectedTipo` — caminho preferido da UI |
...
- `/faturamento/lote/fao` — upload em lote (ZIP multipart em `/upload-zip`; XML em `/upload`), inconsistências, auto-correção (`stNaoPossuiCpf` / INE) e edição manual (CIAP/CID, consulta); download ZIP.
