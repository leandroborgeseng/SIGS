...
5. Permite **entrada a partir da agenda do dia** (`/odonto/agenda` · grade horários × profissional · CONSULTA/ENCAIXE · `AppointmentSlot` → `DentalEncounter.appointmentId`).

**Não-objetivo nesta fase:** agenda TR completa (cadastro livre de tipos de item, salas, grade municipal compartilhada), prótese, telemonitoramento, atestados — entram em ondas posteriores.
