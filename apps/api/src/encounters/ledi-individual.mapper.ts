...
export function buildIndividualEncounterLediPayload(input: {
  ...
  const tipo = resolveTipoAtendimento(input.encounterType);
  ...
  const child: LediIndividualChild = {
    ...
    ciap2MotivoConsulta: input.clinical.ciapCodes,
    problemaCondicaoAvaliada: {
      cid10: input.clinical.cidCodes,
      ciaps: input.clinical.ciapCodes,
    },
    medicoes: {
