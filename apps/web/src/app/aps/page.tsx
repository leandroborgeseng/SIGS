...
type Row = {
  ...
  care?: { faiOrigin?: boolean; tipoAtendimento?: number };
};
...
export default function ApsPage() {
  ...
  async function load() {
    const qs = facilityId ? `?facilityId=${facilityId}&origin=fai` : '?origin=fai';
  ...
  async function open(e: FormEvent) {
    ...
    setBusy(true);
    try {
      const row = await api<{ id: string }>('/v1/encounters', {
        method: 'POST',
        json: {
          patientId,
          facilityId,
  ...
  return (
    <AppShell helpId="aps.atendimento">
      <PageHeader
        title="Atendimento APS"
        description="Ficha individual Siaps-ready (LEDI FAI tipo 4) · Onda 1"
        actions={
          <>
            ...
            <Link className="btn ghost" href="/atendimento">
              Fila SOAP
            </Link>
          </>
        }
      ...
      <section className="card" style={{ marginBottom: 16 }}>
        ...
        <form onSubmit={open} className="grid-form">
          ...
          <button className="btn" type="submit" disabled={busy}>
            {busy ? 'Abrindo…' : 'Abrir atendimento APS'}
          </button>
        </form>
      </section>

      <section className="card">
        <h2 style={{ marginTop: 0 }}>Atendimentos FAI</h2>
