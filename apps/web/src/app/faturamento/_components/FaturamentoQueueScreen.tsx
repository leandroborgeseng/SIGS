...
export type FaturamentoQueueConfig = {
  helpId: string;
  title: string;
  description: string;
  queuePath: string;
...
function FaturamentoQueueInner({ config }: { config: FaturamentoQueueConfig }) {
  ...
  const load = useCallback(
    async (opts?: {
      ...
      try {
        ...
        const res = await api<QueueResponse>(`${config.listEndpoint}?${qs}`);
        setData(res);
        if (opts?.forceSync) {
          const cap =
    ...
    [competencia, facilityId, bucket, config.listEndpoint],
  );
  ...
  }, [competencia, facilityId, focusEncounterId, load, config.syncEndpoint]);
  ...
  function clearFocus() {
    ...
    const q = qs.toString();
    router.replace(q ? `${config.queuePath}?${q}` : config.queuePath);
  }
  ...
  return (
    <AppShell helpId={config.helpId}>
      <PageHeader
        title={config.title}
        description={config.description}
        actions={
          <>
            <HelpLink id={config.helpId} />
            <Link className="btn ghost" href={config.clinicalHref}>
              {config.clinicalLabel}
            </Link>
            <Link className="btn ghost" href={config.loteHref}>
              {config.loteLabel}
            </Link>
            <button
              className="btn ghost"
        ...
        {t && (t.ok > 0 || t.ready > 0) ? (
          ...
          >
            <div style={{ fontSize: 13, lineHeight: 1.45 }}>
              ...
              {' · '}
              para XML importado / ZIP de envio, use o lote LEDI {config.loteKindShort} (upload →
              corrigir → baixar conformes).
            </div>
            <Link className="btn" href={config.loteHref}>
          ...
          <div className="queue-empty">
            {error ? (
              ...
              <>
                ...
                <p className="muted">
                  Para XML importado / correção em lote LEDI, use o lote {config.loteKindShort}.
                </p>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                  <Link className="btn" href={config.loteHref}>
                    {config.loteLabel}
...
export function FaturamentoQueuePage({ config }: { config: FaturamentoQueueConfig }) {
  return (
    <Suspense
      fallback={
        <AppShell helpId={config.helpId}>
          <PageHeader
            ...
            actions={<HelpLink id={config.helpId} />}
          />
        </AppShell>
      }
    >
      <FaturamentoQueueInner config={config} />
    </Suspense>
  );
}
