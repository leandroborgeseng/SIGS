...
- **etapa_atual:** Hotfix Safari File API — ZIP fatia a fatia (FileReader+XHR), sem montar na RAM
...
- **Relatório do que falta (FAI/FAO/PROC):** após o autofix, `GET /v1/dental/ledi/batches/:id/pending-report` (JSON + `?format=csv|md` + `?severity=`). UI **Relatório do que falta** — tabela + CSV/MD. CPF mascarado; BLOCKER = Siaps; o resto = qualidade/Previne. Sem R$ pedagógico.
...
_Atualizado em 2026-08-14 (Safari ZIP: fatia FileReader+XHR, sem Uint8Array do arquivo inteiro)_
